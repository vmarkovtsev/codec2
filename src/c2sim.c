...
void synth_one_frame(fft_cfg fft_inv_cfg, short buf[], MODEL *model, float Sn_[], float Pn[], int prede, float *de_mem, float gain);
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    fft_fwd_cfg = fft_new(FFT_ENC, 0); /* fwd FFT,used in several places   */
    fft_inv_cfg = fft_new(FFT_DEC, 1); /* inverse FFT, used just for synth */
	...
	if (lpc_model) {
	    ...
	    */
		...
		for(i=0; i<LPC_ORD; i++) {
		    f = (4000.0/PI)*lsps[i];
		    mel[i] = floorf(100.0*log10f(1.0 + f/700.0) + 0.5);
		}
		...
		for(i=0; i<LPC_ORD; i++) {
		    f_ = 700.0*( powf(10.0, (float)mel[i]/100.0) - 1.0);
