...
typedef struct {
    ...
    fft_cfg  fft_cfg;           /* kiss FFT config              */
} NLP;
...
/*---------------------------------------------------------------------------*\
  ...
  Initialization function for NLP pitch estimator.
...
void *nlp_create()
{
    ...
    nlp->fft_cfg = fft_new (PE_FFT_SIZE, 0);
