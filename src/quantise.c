...
void lpc_post_filter(fft_cfg fft_fwd_cfg, MODEL *model, COMP Pw[], float ak[], int order, int dump)
{
    int   i;
    ...
    init_comp_array(x, FFT_ENC);
    
    for(i=0; i<=order; i++)
	x[i].real = ak[i];
    fft_do(fft_fwd_cfg, x, Aw);
    ...
    for(i=0; i<=order; i++) {
        x[i].real = ak[i] * powf(LPCPF_GAMMA, (float)i);
    }
    fft_do(fft_fwd_cfg, x, Ww);
...
float decode_amplitudes(fft_cfg  fft_fwd_cfg, 
			MODEL *model, 
			float  ak[],
		        int    lsp_indexes[], 
		        int    energy_index,
